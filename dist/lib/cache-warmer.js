...
async function getRevenue() {
    ...
    return reports.reduce((sum, report) => sum + report.writeOffs + report.ninetyPlus, 0);
}
...
async function getGrowthRate() {
    ...
    const currentTotal = (currentMonth._sum.writeOffs || 0) + (currentMonth._sum.ninetyPlus || 0);
    const lastTotal = (lastMonth._sum.writeOffs || 0) + (lastMonth._sum.ninetyPlus || 0);
    if (lastTotal === 0)
        return 0;

...
import { receiveFromNotificationQueue, deleteBatchFromQueue } from '@/lib/queue/sqs';
 ...
 */
async function createInAppNotifications(type, data, userIds) {
 ...
 */
async function processBatchNotifications(messages) {
    const successful = [];
    const failed = [];
    // Process messages in parallel with a concurrency limit
    const results = await Promise.allSettled(messages.map(message => processNotificationMessage(message)));
    // Collect results
...
async function processNotificationMessage(message) {
    try {
        ...
        const inAppCount = await createInAppNotifications(notification.type, notification.data || {}, notification.userIds);
        ...
        const pushResults = await Promise.allSettled(subscriptions.map(subscription => sendNotificationWithRetry(subscription, notificationContent)));

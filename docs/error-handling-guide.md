...
This document outlines the standardized error handling patterns implemented across the codebase. Following these patterns will ensure consistent error handling, better debugging, and improved user experience.
...
class AppError extends Error {
    ...
    public readonly isOperational: boolean; // Whether this is an expected error
...
Logs errors with consistent structure and severity levels (DEBUG, INFO, WARN, ERROR, FATAL).
...
Converts any error to a standardized server action response.
...
Executes operations that shouldn't fail the main flow, with logging and fallback values.
...
1. **Use Specific Error Classes**: Choose the most appropriate error class for each situation.
...
3. **Handle Non-critical Operations**: Use `tryNonCritical` for operations that shouldn't fail the main flow.
...
5. **Validate Early**: Validate inputs at the beginning of functions to fail fast.

6. **Log Appropriately**: Use the correct log level based on error severity.

7. **Don't Expose Sensitive Information**: Sanitize error messages and contexts before sending to clients.

...
const filesWithReferences = filesToCheck.filter((file) => {
  ...
  const hasSQSReference =
    content.includes("sqs") ||
    content.includes("SQS") ||
    content.includes("queue");
  const hasWorkerReference =
    content.includes("worker") || content.includes("Worker");

  console.log(
    `${file}: ${hasSQSReference || hasWorkerReference ? "HAS REFERENCES" : "NO REFERENCES"}`,
  );

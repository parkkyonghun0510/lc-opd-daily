...
export function PermissionDebugger() {
  ...
  const groupedPermissions: Record<string, Permission[]> = {
    Admin: filteredPermissions.filter(
      (p) => p.includes("ADMIN") || p.includes("MANAGE"),
    ),
    ...
    Dashboard: filteredPermissions.filter(
      (p) =>
        p.toLowerCase().includes("dashboard") ||
        p.toLowerCase().includes("analytics"),
    ),
    Other: filteredPermissions.filter(
      (p) =>
        !p.includes("ADMIN") &&
        !p.includes("MANAGE") &&
        !p.includes("REPORT") &&
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <Input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && checkCustomPermission()
                  }
                />
              ...
              {checkResult !== null && (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      Permission <strong>{customPermission}</strong> is{" "}
                      {checkResult ? "granted" : "denied"} for role{" "}
                      {debugInfo.role}
                    </span>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  ...
                  <p className="text-sm">
                    {debugInfo.user?.name || "Not authenticated"}
                  </p>
                </div>
                <div className="space-y-1">

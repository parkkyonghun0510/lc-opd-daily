...
export function EventsChart({ metrics, height = 300 }: EventsChartProps) {
  ...
  useEffect(() => {
    ...
    const spacing =
      (width - barWidth * eventTypes.length) / (eventTypes.length + 1);

    // Find max value for scaling
    const maxValue = Math.max(
      ...(Object.values(metrics.events?.byType || {}) as number[]),
    );
    ...
    eventTypes.forEach((eventType, index) => {
      ...
      const barHeight =
        maxValue > 0 ? (count / maxValue) * chartHeight * 0.9 : 0;
      ...
      const displayName =
        eventType.length > 10 ? eventType.substring(0, 8) + "..." : eventType;

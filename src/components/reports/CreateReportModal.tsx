      ...
      <DialogContent className="sm:max-w-[425px] max-h-[90vh] overflow-y-auto dark:bg-gray-800 dark:border-gray-700">
        ...
        <div className="grid gap-4 py-4">
          ...
          <input
            ...
            value={
              formData.title ||
              `${reportType === "plan" ? "Plan" : "Actual"} Report - ${format(new Date(), "yyyy-MM-dd")}`
            }
          />
          {errors.title && (
          ...
          <div className="grid gap-2">
            <Label htmlFor="branch" className="dark:text-gray-200 text-sm">
              Branch
            </Label>
            <BranchSelector
              id="branch"
          ...
          <div className="grid gap-2">
            <div className="flex items-center justify-between">
              <Label htmlFor="writeOffs" className="dark:text-gray-200 text-sm">
                Write-offs
              </Label>
            ...
            {validationRules?.writeOffs.requireApproval &&
              formData.writeOffs > 0 && (
                <p className="text-xs text-amber-600 dark:text-amber-400">
                  Write-offs require approval and will be sent to the approval
                  queue
                </p>
              )}
          </div>
          ...
          <div className="grid gap-2">
            ...
            <KHCurrencyInput
              ...
              onValueChange={(value) =>
                updateField("ninetyPlus", Number(value))
              }
              className="dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 text-sm"
            />
            {errors.ninetyPlus && (
            ...
            {validationRules?.ninetyPlus.requireApproval &&
              formData.ninetyPlus > 0 && (
                <p className="text-xs text-amber-600 dark:text-amber-400">
                  90+ days require approval and will be sent to the approval
                  queue
                </p>
              )}
          </div>
                  ...
                  {validationRules?.comments.required && (
                    <p className="text-xs text-muted-foreground dark:text-gray-400">
                      {formData.initialComment
                        ...
                        : 0}
                      /{validationRules.comments.minLength} characters minimum
                    </p>
                  )}
                ...
                planData.writeOffsPlan === null && (
                  <Alert className="mt-3 bg-amber-50 border-amber-200 dark:bg-amber-900/30 dark:border-amber-800">
                    ...
                    <AlertDescription className="text-amber-700 text-xs dark:text-amber-300">
                      No morning plan found for this date and branch. Please
                      ensure a plan report is submitted first.
                    </AlertDescription>
                  </Alert>
                )}
            </div>
        ...
        {reportType === "actual" &&
          planData.planReportId &&
          planData.planReportStatus !== "approved" && (
            <Alert variant="destructive" className="mt-4">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                The plan report for this date must be approved before you can
                submit an actual report.
              </AlertDescription>
            </Alert>
          )}

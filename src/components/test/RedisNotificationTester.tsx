      ...
      "*": (event) => {
        ...
        if (event.type === "notification") {
          toast(event.title || "New Notification", {
            description:
              event.message || event.body || "You have a new notification",
          });
        }
      },
            ...
            >
              {redisStatus === "connected" ? (
                <span className="flex items-center gap-1">
                  <CheckCircle className="h-4 w-4" /> Redis Connected
                </span>
              ) : redisStatus === "checking" ? (
                <span className="flex items-center gap-1">
                  <Loader2 className="h-4 w-4 animate-spin" /> Checking Redis
                </span>
              ) : (
                <span className="flex items-center gap-1">
                  <AlertCircle className="h-4 w-4" /> Redis Disconnected
                </span>
              )}
            </Badge>
          </CardTitle>
          <CardDescription>
            Test the Redis notification system by sending and receiving
          ...
          <CardContent>
            {!metrics ? (
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="border rounded-md p-3">
                      ...
                      <div className="text-2xl font-bold">
                        {metrics.metrics?.processingCount || 0}
                      </div>
                    </div>
                    <div className="border rounded-md p-3">
                      <div className="text-xs text-muted-foreground">
                      ...
                      <div className="text-2xl font-bold">
                        {metrics.metrics?.historyCount || 0}
                      </div>
                    </div>
                    <div className="border rounded-md p-3">
                      <div className="text-xs text-muted-foreground">
                ...
                {metrics.redisInfo?.history &&
                  metrics.redisInfo.history.length > 0 && (
                    <div>
                      <h3 className="text-sm font-medium mb-2">

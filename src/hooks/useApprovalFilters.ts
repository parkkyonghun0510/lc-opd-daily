  ...
  const filterReports = useCallback(
    (reports: ProcessedReport[]) => {
      return reports.filter((report) => {
        if (
          filterState.branchFilter !== "all" &&
          report.branchId !== filterState.branchFilter
        ) {
          return false;
        }

        if (
          filterState.reportTypeFilter !== "all" &&
          report.reportType !== filterState.reportTypeFilter
        ) {
          return false;
        }
        ...
        if (filterState.dateRange.from || filterState.dateRange.to) {
          ...
          if (
            ...
            reportDate < filterState.dateRange.from
          ) {
            return false;
          }
          if (
            filterState.dateRange.to &&
            reportDate > filterState.dateRange.to
          ) {
            return false;
          }
        }

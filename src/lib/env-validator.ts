...
export class EnvironmentValidator {
  ...
  public validateEnvironment(strict: boolean = false): ValidationResult {
    ...
    try {
      ...
      this.validateVapidConfig(errors);
      ...
      if (!validation.success) {
        ...
        this.validatedEnv = validation.data;
      }
  ...
  private validateDragonflyConfig(errors: string[], warnings: string[]): void {
    const dragonflyUrl = process.env.DRAGONFLY_URL || process.env.REDIS_URL;
   ...
   */
  private validateVapidConfig(errors: string[]): void {
    ...
    if (!publicKey || publicKey.trim().length === 0) {
      ...
      // Validate VAPID public key format (Base64URL)
      if (!this.isValidVapidKey(publicKey)) {
    ...
    if (!privateKey || privateKey.trim().length === 0) {
      ...
      // Validate VAPID private key format (Base64URL)
      if (!this.isValidVapidKey(privateKey)) {
  ...
  private performCustomValidations(errors: string[], warnings: string[]): void {
    ...
    if (process.env.NODE_ENV === 'production') {
      if (!process.env.DRAGONFLY_URL && !process.env.REDIS_URL) {

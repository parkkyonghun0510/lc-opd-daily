...
export function generatePrometheusMetrics(): string {
  ...
  // Connection metrics
  lines.push(
    "# HELP sse_connections_total Total number of SSE connections since last reset",
  );
  ...
  lines.push(
    "# HELP sse_connections_peak Peak number of concurrent SSE connections",
  );
  ...
  lines.push(
    "# HELP sse_connections_by_user Number of active connections per user",
  );
  ...
  // Event metrics
  lines.push(
    "# HELP sse_events_total Total number of SSE events sent since last reset",
  );
  ...
  // Error metrics
  lines.push(
    "# HELP sse_errors_total Total number of SSE errors since last reset",
  );
  ...
  // Performance metrics
  lines.push(
    "# HELP sse_event_processing_time_average Average event processing time in milliseconds",
  );
  lines.push("# TYPE sse_event_processing_time_average gauge");
  lines.push(
    `sse_event_processing_time_average ${metrics.performance.averageEventProcessingTime || 0}`,
  );

  lines.push(
    "# HELP sse_event_processing_count Number of events processed for performance measurement",
  );
  lines.push("# TYPE sse_event_processing_count counter");
  lines.push(
    `sse_event_processing_count ${metrics.performance.eventProcessingCount}`,
  );

  // Add timestamp
  lines.push(
    "# HELP sse_metrics_last_reset Timestamp of the last metrics reset",
  );

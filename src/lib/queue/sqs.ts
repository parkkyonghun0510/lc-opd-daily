...
const sqsClient = new SQSClient({
  ...
  credentials: {
    accessKeyId:
      process.env.AWS_ACCESS_KEY_ID || process.env.SQS_AWS_ACCESS_KEY_ID || "",
    secretAccessKey:
      process.env.AWS_SECRET_ACCESS_KEY ||
      process.env.SQS_AWS_SECRET_ACCESS_KEY ||
      "",
  },
 ...
 */
export async function sendToNotificationQueue(
  ...
  try {
    if (process.env.NODE_ENV === "production") {
      ...
      console.log(
        `Sending ${message.type} notification to ${message.userIds?.length || 0} recipients`,
      );
    } else {
 ...
 */
export async function sendBatchToNotificationQueue(

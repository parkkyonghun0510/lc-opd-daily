...
export class RedisLoadBalancer {
  ...
  private initializeInstances(configs: RedisInstanceConfig[]): void {
    ...
    configs.forEach((config, index) => {
      ...
      try {
        ...
        this.log(
          `Failed to initialize Redis instance at index ${index}:`,
          error,
        );
      }
    });
      ...
      async ([instanceId, instance]) => {
        try {
          ...
          if (instance.health.consecutiveErrors >= this.errorThreshold) {
            ...
            this.log(
              `Instance ${instanceId} marked as unhealthy after ${instance.health.consecutiveErrors} consecutive errors`,
            );
          }
  ...
  private getNextInstance(): { client: Redis; instanceId: string } | null {
    ...
    this.currentInstanceIndex =
      (this.currentInstanceIndex + 1) % weightedInstances.length;
    ...
    try {
      ...
      if (instanceData) {
        ...
        if (instanceData.health.consecutiveErrors >= this.errorThreshold) {
          ...
          this.log(
            `Instance ${instance.instanceId} marked as unhealthy after operation error`,
          );
        }
      }

      this.log(
        `Error executing Redis operation on instance ${instance.instanceId}:`,
        error,
      );

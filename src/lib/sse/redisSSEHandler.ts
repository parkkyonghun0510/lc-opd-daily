...
class RedisSSEHandler {
  ...
  constructor(redisUrl: string, redisToken: string) {
    ...
    console.log(
      `[SSE] Redis-backed SSE handler initialized with instance ID: ${this.instanceId}`,
    );
  }
  ...
  async addClient(
    ...
    try {
      ...
      console.log(
        `[SSE] Client connected: ${id} for user: ${userId}. Total local: ${this.clients.size}`,
      );
    } catch (error) {
  ...
  async removeClient(id: string) {
    ...
    if (client) {
      ...
      try {
        ...
        console.log(
          `[SSE] Client disconnected: ${id} for user: ${client.userId}. Duration: ${duration}s. Total local: ${this.clients.size}`,
        );
      } catch (error) {
  ...
  async updateClientActivity(id: string) {
    ...
    if (client) {
      ...
      if (timeSinceLastUpdate > 300000) {
        // 5 minutes
        try {
          const clientData = await this.redis.hget(
            `sse:clients:${client.userId}`,
            id,
          );
          if (clientData) {
          ...
          console.error(
            "[SSE] Error updating client activity in Redis:",
            error,
          );
        }
      }
  ...
  async sendEventToUser(userId: string, eventType: string, data: any) {
    ...
    if (localSentCount > 0) {
      console.log(
        `[SSE] Sent '${eventType}' event to user ${userId} (${localSentCount} local connections)`,
      );
    }
  ...
  async broadcastEvent(eventType: string, data: any) {
    ...
    console.log(
      `[SSE] Broadcast '${eventType}' event to all clients (${this.clients.size} local)`,
    );
  ...
  private async cleanupInactiveConnections() {
    ...
    if (inactiveIds.length > 0) {
      console.log(
        `[SSE] Cleaning up ${inactiveIds.length} inactive local connections`,
      );
      ...
      try {
        ...
        for (const [userId, count] of entries) {
          if (parseInt(count as string, 10) <= 0) {
            ...
            console.log(
              `[SSE] Cleaned up Redis entries for user ${userId} with no active connections`,
            );
          }
        }
      } catch (error) {
  ...
  async getStats() {
    try {
      ...
      for (const client of this.clients.values()) {
        localUserCounts.set(
          client.userId,
          (localUserCounts.get(client.userId) || 0) + 1,
        );
      }
      ...
      return {
        ...
        globalUniqueUsers: globalUserCounts
          ? Object.keys(globalUserCounts).length
          : 0,

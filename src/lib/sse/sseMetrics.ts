...
class SSEMetrics {
  ...
  recordConnection(userId: string) {
    ...
    this.metrics.connections.byUser[userId] =
      (this.metrics.connections.byUser[userId] || 0) + 1;
  }
  ...
  recordDisconnection(userId: string) {
    ...
    if (this.metrics.connections.byUser[userId]) {
      this.metrics.connections.byUser[userId] = Math.max(
        0,
        this.metrics.connections.byUser[userId] - 1,
      );
    }
  }
  ...
  recordEvent(eventType: string, userId: string) {
    ...
    this.metrics.events.byUser[userId] =
      (this.metrics.events.byUser[userId] || 0) + 1;
  }
  ...
  recordEventProcessingTime(timeMs: number) {
    ...
    if (
      this.metrics.performance.eventProcessingTime.length >
      this.MAX_PERFORMANCE_SAMPLES
    ) {
  ...
  private calculateAverageProcessingTime() {
    ...
    const sum = this.metrics.performance.eventProcessingTime.reduce(
      (a, b) => a + b,
      0,
    );

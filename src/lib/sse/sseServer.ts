...
export class SSEHandler {
  ...
  sendEventToUser(
    ...
    if (sentCount > 0) {
      ...
      console.log(
        `[SSE] Sent '${eventType}' event to user ${userId} (${sentCount} connections) in ${processingTime.toFixed(2)}ms`,
      );
    }
  ...
  sendEventToRole(
    ...
    if (sentCount > 0) {
      ...
      console.log(
        `[SSE] Sent '${eventType}' event to role ${role} (${sentCount} connections) in ${processingTime.toFixed(2)}ms`,
      );
    }
  ...
  broadcastEvent(
    ...
    console.log(
      `[SSE] Broadcast complete: Sent to ${this.clients.size} connections (${userIds.size} unique users) in ${processingTime.toFixed(2)}ms`,
    );
...
export function createSSEHandler(options: {
  ...
  const { onConnect, onDisconnect, pingInterval = 10000 } = options;
  ...
  return async function handleSSE(request: NextRequest) {
    ...
    const clientInfo =
      url.searchParams.get("clientInfo") ||
      request.headers.get("user-agent") ||
      "";
    const role = url.searchParams.get("role") || "user";

    console.log(
      `[SSE] New connection request from user: ${userId}, client: ${clientType}, role: ${role}`,
    );

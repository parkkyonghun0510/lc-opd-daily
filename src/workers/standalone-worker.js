...
if (!queueUrl) {
  console.error(
    "AWS_SQS_NOTIFICATION_QUEUE_URL environment variable is not set",
  );
  process.exit(1);
}
  ...
  try {
    ...
    if (notification.data && notification.type) {
      ...
      if (notification.data.title)
        notificationContent.title = notification.data.title;
      if (notification.data.body)
        notificationContent.body = notification.data.body;
      if (notification.data.icon)
        notificationContent.icon = notification.data.icon;
      if (notification.data.url)
        notificationContent.url = notification.data.url;

      // Add type-specific content if not provided
      switch (notification.type) {
        case "REPORT_SUBMITTED":
          if (!notification.data.title)
            notificationContent.title = "New Report Submitted";
          if (!notification.data.body)
            notificationContent.body = `A new report has been submitted by ${notification.data.submitterName || "a user"} and requires review.`;
          break;
        case "REPORT_APPROVED":
          if (!notification.data.title)
            notificationContent.title = "Report Approved";
          if (!notification.data.body)
            notificationContent.body = `Your report has been approved by ${notification.data.approverName || "a manager"}.`;
          break;
        case "REPORT_REJECTED":
          if (!notification.data.title)
            notificationContent.title = "Report Rejected";
          if (!notification.data.body)
            notificationContent.body = `Your report has been rejected by ${notification.data.approverName || "a manager"}.`;
          break;

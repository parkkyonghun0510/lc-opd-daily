...
const server = http.createServer((req, res) => {
  ...
  if (req.url.startsWith("/sse")) {
    ...
    console.log(
      `Client connected to SSE endpoint - User ID: ${userId}, Role: ${userRole}`,
    );
        ...
        sendToUser: (userId, eventType, data) => {
          ...
          for (const client of clients.values()) {
            if (client.userId === userId) {
              console.log(
                `Sending ${eventType} event to user ${userId}:`,
                data,
              );
        ...
        sendToRole: (role, eventType, data) => {
          ...
          for (const client of clients.values()) {
            if (client.userRole === role) {
              console.log(
                `Sending ${eventType} event to ${role} ${client.userId}:`,
                data,
              );
              client.response.write(
                `event: ${eventType}\ndata: ${JSON.stringify(data)}\n\n`,
  ...
  // Endpoint to trigger custom events
    ...
    if (eventType.startsWith("report")) {
      eventData.reportId =
        params.get("reportId") || Math.random().toString(36).substring(2, 9);
      eventData.branchId =
        params.get("branchId") ||
        "branch-" + Math.random().toString(36).substring(2, 5);
...
server.listen(PORT, () => {
  ...
  console.log(
    `- SSE endpoint: http://localhost:${PORT}/sse?userId=user1&role=admin`,
  );
  console.log(`- Web client: http://localhost:${PORT}/`);
  console.log("\nAvailable endpoints:");
  console.log(
    `- General events: http://localhost:${PORT}/trigger-event?type=notification&message=Test`,
  );
  console.log(
    `- Role-based events: http://localhost:${PORT}/trigger-event?type=notification&message=Test&target=role&targetId=admin`,
  );
  console.log(
    `- User-specific events: http://localhost:${PORT}/trigger-event?type=notification&message=Test&target=user&targetId=user1`,
  );
  console.log(
    `- Report approval: http://localhost:${PORT}/report-approval?action=approve&reportId=123&branchName=TestBranch`,
  );
  console.log(
    `- Report rejection: http://localhost:${PORT}/report-approval?action=reject&reportId=123&branchName=TestBranch&comments=Needs%20revision`,
  );
});
